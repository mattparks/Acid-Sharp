...
namespace AcidSharp.Tests
{
    class MainWorld : IModule
    {
        ...
        public override void Update()
        {
            ...
            _factorSunrise = Maths.Clamp(-((float)Math.Sin(2.0f * Math.PI * DayFactor) - 1.0f) / 2.0f, 0.0f, 1.0f);
            _factorShadow = Maths.Clamp(1.7f * (float)Math.Sin(2.0f * Math.PI * DayFactor), 0.0f, 1.0f);;
